...
class TestAccountTransactionTemplate:
    ...
    def test_accounts_transaction_template(self, db_session: Session):
        ...
        with allure.step("Создание тестовых данных в базе данных: счета Stan и Bob"):
            stan = AccountTransactionTemplate(username=f"Stan_{DataGenerator.generate_random_int(10)}", balance=1000)
            bob = AccountTransactionTemplate(username=f"Bob_{DataGenerator.generate_random_int(10)}", balance=500)
        ...
        def transfer_money(session, from_account, to_account, amount):
            with allure.step(" Получаем счета"):
                from_account = session.query(AccountTransactionTemplate).filter_by(username=from_account).one()
                to_account = session.query(AccountTransactionTemplate).filter_by(username=to_account).one()
        ...
        try:
            with allure.step("Выполняем перевод 200 единиц от stan к bob"):
                transfer_money(db_session, from_account=stan.username, to_account=bob.username, amount=200)
